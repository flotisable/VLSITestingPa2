...
ATPG::wptr ATPG::find_pi_assignment(const wptr object_wire, const int& object_level) {
  ...
  /* if not PI, backtrace to PI  Fig 9.3, 9.4, 9.5*/
    ...
    //--------------------------------- hole -------------------------------------------
        new_object_wire = ( object_level ) ?  find_hardest_control( object_wire->inode.front() ) :
...
bool ATPG::trace_unknown_path(const wptr w) {
  ...
  wptr wtemp;
	...
	//------------------------------------- hole ---------------------------------------
  if( w->flag | OUTPUT && w->value == U ) return true; // PO
  ...
  for( i = nout - 1 ; i >= 0 ; --i )
  {
     wtemp = w->onode[i]->owire.front();

     if( wtemp->value == U && trace_unknown_path( wtemp ) )
       return true;
  }
